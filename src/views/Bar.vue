<template lang="">
    <div 
      ...
      <lg:px-10
      ">
      <img class="w-25 inline float-left <md:w-15" src="../assets/img/logo.png" alt="logo" />
      <div class="right flex items-center">
        <!-- 登录头像 -->
        <div class="avatar_box mr-10">
          <!-- 未登录状态 -->
          <div class="bg-gray-200 rounded-full w-13 h-13 flex justify-center items-center cursor-pointer" v-if="!login.isLogin" @click="login.showLoginView = true">
            <span class="text-blue-400" >登录</span>
          </div>
          <!-- 登录状态 -->
          <div v-else>
            <el-popover v-model:visible="login.showUserInfo" placement="bottom" :width="240">
              <div relative>
                <ul>
                  <li class="my-5">
                ...
                <i class="fa-solid fa-xmark absolute top-5 right-5 text-xl cursor-pointer" @click="login.showUserInfo = false"></i>
                <el-button class="float-right" @click="logoutFn">退出登录</el-button>
              </div>
              <template #reference>
                <el-avatar class="mr-3 w-13 h-13" @click="login.showUserInfo = !login.showUserInfo" :src="login.imgUrl + login.loginInfo.avatar" />
              </template>
            </el-popover>
          </div>
        </div>
    ...
    <div
        ...
        v-show="menuShow"
    >
        <ul class="flex flex-col items-center justify-center h-full text-4xl" style="font-family: font1">
